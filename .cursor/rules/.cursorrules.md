...
Note: Scope is strictly single-coin (BTC/USDT). Do not add altcoins unless explicitly requested.

## Tech Stack
- **Language**: Python 3.9+
- **Framework**: python-telegram-bot 20.7 (async handlers)
- **Exchange API**: ccxt (Binance) — consider ccxt.pro (WebSocket) for low-latency
...
- **Trading Style**: Swing trading (4-24 hours)
 - **Day Trading Mode**: Optional mode (1m timeframe) gated by plan approval
...
7. Initialize logging **once** (in `main.py`). Do not call `logging.basicConfig` in modules.
8. All Telegram handlers and long-running tasks must be **non-blocking**; avoid blocking I/O in the event loop.
9. Do not start implementation of new features without explicit user approval after plan delivery.
...
- Prefer WebSocket streams via `ccxt.pro` for real-time price/orderbook when low latency is required (speculative, optional).
...
- Respect rate limits; implement retry with backoff for 429/5xx
- Ensure messages stay under 4096 chars; truncate or split when needed
...
- Use caching (TTL ~5 minutes); prefer async HTTP client in async contexts
...
- Parallelize independent requests (OHLCV, FNG, 24h stats, orderbook)
- Prefer WebSockets for price/orderbook when feasible
 - Keep REST-safe fallback with caching/jitter when WebSocket is not enabled
...
- [ ] Logging statements added (no duplicate basicConfig in modules)
...
3.1 Prefer concrete code edits (diff with 2-3 lines context), avoid high-level descriptions only
3.2 Use Python async/await patterns; avoid blocking calls in handlers
